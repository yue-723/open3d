...
def startButton_Click():
    ...
    for i in range(7):
        if listOfVar[i].get() != "----":
            materialName.append(listOfVar[i].get())
    if filePath == "":
        statusUpdate("Please select file !")
        return
    if len(materialName) == 0:
        statusUpdate("Select at least one material !")
        return
    statusUpdate("")
...
Param_label = tk.Label(root, text=' Material :  ', font=('Microsoft JhengHei UI', 10, 'bold'))
...
# Drop down list
OptionList = ["----", "none", "wafer", "polar", "glue", "glass"]
Dropdown_list = list()
listOfVar = list()
for i in range(7):
    listVar = tk.StringVar(root)
    listVar.set(OptionList[0])
    opt = tk.OptionMenu(root, listVar, *OptionList)
    opt.config(width=4, font=('Microsoft JhengHei UI', 9))
    opt.place(x=85+i*75, y=32)
    Dropdown_list.append(opt)
    listOfVar.append(listVar)
